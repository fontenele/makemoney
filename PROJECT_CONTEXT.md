...
- M6 is closed. M7.1–M7.106 incrementally implement the new-listing research pipeline through durable catalog detection and observation, descriptive market/book analyses and APIs, comparable price-path and variability cohorts, and locally exposed exact cost-adjusted checkpoint round-trip cohort research.
...
M7.82 exposes that bounded explicit-threshold classification cohort through a validated local read-only API.

M7.83 purely calculates the exact median maximum widening magnitude among classifications that reached the caller-supplied threshold.

M7.84 composes that magnitude statistic over the same bounded durable classification sample used by M7.81.

M7.85 exposes that durable magnitude statistic at `GET /new-listings/top-of-book/spread/classification/magnitudes` with the same mandatory explicit threshold and bounded cohort inputs.

M7.86 purely calculates the median T+0-to-first-widening duration among threshold-qualified spread classifications with an independent sample size and strict checkpoint validation.

M7.87 composes that timing statistic over the shared bounded durable spread-classification sample without adding HTTP exposure or persistence.

M7.88 exposes that durable timing statistic at `GET /new-listings/top-of-book/spread/classification/timing` with the same mandatory explicit threshold and bounded cohort inputs.

M7.89 purely derives observed checkpoint-price high, low, and maximum causal peak-to-trough drawdown with exact decimal arithmetic and no score or prediction.

M7.90 composes those price-path statistics on demand from one detection's durable completed checkpoint timeline, preserving not-found and unavailable-without-T+0 semantics without adding HTTP exposure.

M7.91 exposes those durable per-detection price-path statistics at local read-only `GET /new-listings/:provider/:symbol/price-path`, with validated identity and explicit not-found and unavailable responses.

M7.92 purely aggregates comparable price-path timing and positive-drawdown statistics across a validated cohort without comparing absolute prices between distinct assets.

M7.93 composes that price-path aggregate on demand over the bounded recent durable T+0-eligible observation cohort without adding HTTP exposure or persistence.

M7.94 exposes that durable aggregate at local read-only `GET /new-listings/price-path` with bounded provider and limit input.

M7.95 purely calculates exact consecutive checkpoint simple returns, their average absolute magnitude, and the earliest maximum absolute transition. Sparse uneven checkpoint timing remains explicit; the result is not annualized volatility, a forecast, or a score.

M7.96 composes that variability calculation on demand from one detection's durable completed checkpoint timeline while preserving explicit not-found and missing-T+0 semantics.
...
M7.100 exposes that durable aggregate at local read-only `GET /new-listings/variability` with bounded provider and limit input.

M7.101 purely calculates one explicitly selected checkpoint round trip from stored ask to later stored bid, including observed spread, caller-supplied fees on both sides, and adverse slippage without placing or simulating an order ledger.

M7.102 composes that calculation on demand from one known detection's durable top-of-book timeline. It validates the selected forward pair and cost rates before repository access, preserves not-found semantics, and returns unavailable when either selected book is absent; it adds no route or trading behavior.
...
M7.106 exposes that bounded aggregate at local read-only `GET /new-listings/round-trip`. It requires explicit entry/exit labels and fee/slippage rates, supports only the established bounded limit and Binance provider inputs, and adds no defaults, pair search, ranking, or execution behavior.
