...
| `GET`    | `/new-listings/top-of-book/imbalance/evolution`                   | Average exact T+0-relative displayed imbalance change by checkpoint for a recent durable detection cohort   | Local, read-only; optional `limit=1..100` (default `50`) and `provider=binance`; excludes detections without a usable T+0 imbalance baseline and reports available/unavailable checkpoint change coverage; descriptive only                                                                         |
| `GET`    | `/new-listings/top-of-book/spread/evolution`                      | Average exact T+0-relative spread-basis-point change by checkpoint for a recent durable detection cohort    | Local, read-only; optional `limit=1..100` (default `50`) and `provider=binance`; includes only detections with a usable stored T+0 book and reports independent checkpoint samples; positive averages mean widening and negative averages mean tightening                                             |
| `GET`    | `/new-listings/top-of-book/spread/classification`                 | Spread-widening classification statistics for a recent durable detection cohort                            | Local, read-only; optional `limit=1..100` (default `50`) and `provider=binance`; requires positive decimal `wideningBasisPoints`; reports explicit observed/not-observed counts and exact observed rate; descriptive only                                                                            |
| `GET`    | `/new-listings/top-of-book/spread/classification/magnitudes`      | Median maximum spread widening for a recent durable detection cohort                                        | Local, read-only; optional `limit=1..100` (default `50`) and `provider=binance`; requires positive decimal `wideningBasisPoints`; reports the independent observed-widening sample and a nullable exact median; descriptive only                                                                       |
| `GET`    | `/new-listings/top-of-book/spread/classification/timing`          | Median time from T+0 to first spread widening for a recent durable detection cohort                          | Local, read-only; optional `limit=1..100` (default `50`) and `provider=binance`; requires positive decimal `wideningBasisPoints`; reports the independent observed-widening sample and nullable median milliseconds; descriptive only                                                                   |
...
| `GET`    | `/new-listings/:provider/:symbol/round-trip`                      | Exact cost-adjusted return for one explicitly selected durable top-of-book checkpoint pair                  | Local, read-only; requires `entryLabel`, `exitLabel`, `feeRate`, and `slippageRate`; encode `+` in labels as `%2B` (for example `T%2B0`); returns `400` for invalid input, `404` for an unknown detection, and `503` when either selected book is unavailable; does not model depth or fills             |
| `GET`    | `/new-listings/:provider/:symbol/top-of-book/imbalance`           | Exact displayed bid/ask quote notionals and normalized level-one imbalance across stored checkpoints        | Local, read-only; returns `404` for an unknown detection, an empty array when no book is stored, and `null` imbalance when both displayed quantities are zero; descriptive only and does not trigger collection                                                                                     |
| `GET`    | `/new-listings/:provider/:symbol/top-of-book/imbalance/evolution` | Exact checkpoint imbalance changes relative to the detected symbol's available `T+0` book                   | Local, read-only; returns `404` for an unknown detection and `503` until T+0 imbalance is available; later zero-notional books retain `null` imbalance and change; descriptive only                                                                                                                 |
| `GET`    | `/new-listings/:provider/:symbol/top-of-book/spread/evolution`    | Exact checkpoint spread-basis-point changes relative to the detected symbol's stored `T+0` book             | Local, read-only; returns `404` for an unknown detection and `503` until a T+0 book is available; positive change means widening and negative change means tightening; descriptive only                                                                                                            |
| `GET`    | `/new-listings/:provider/:symbol/top-of-book/spread/classification` | Observed spread-widening classification for one durable detection                                         | Local, read-only; requires positive decimal `wideningBasisPoints`; returns `400` for invalid input, `404` for an unknown detection, and `503` until a T+0 book is available; descriptive only                                                                                                        |
...
| `GET`    | `/new-listings/:provider/:symbol/price-path`                      | Observed checkpoint-price high, low, and maximum causal drawdown for one durable detection                  | Local, read-only; validates the same identity as the observation timeline, returns `404` for an unknown detection and `503` until `T+0` is complete; sparse checkpoint observations only                                                                                                            |
| `GET`    | `/new-listings/:provider/:symbol/variability`                     | Exact non-annualized price variability between consecutive checkpoints for one durable detection           | Local, read-only; validates the same identity as the observation timeline, returns `404` for an unknown detection and `503` until `T+0` is complete; reports average absolute return and the earliest maximum absolute transition                                                                    |
