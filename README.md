...
Local, personal platform for crypto market research, realistic paper trading, and strategy validation. M0 through M6 and M7.1–M7.8 are complete. Market feeds and bounded historical candles are public and unauthenticated; historical candles, capital, fills, equity, statistics, and balances are fictional or research-only, with no exchange-account or real-fund access.
...
| `GET` | `/new-listings` | Recently detected post-baseline Spot/USDT symbols, newest first | Local, read-only; optional `limit=1..100` (default `50`), `provider:symbol` cursor, and inclusive canonical UTC `detectedFrom`/`detectedTo`; returns an empty array when none exist |
