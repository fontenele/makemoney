...
Local, personal platform for crypto market research, realistic paper trading, and strategy validation. M0 through M5 and M6.1–M6.29 are complete. Market feeds and bounded historical candles are public and unauthenticated; historical candles, capital, fills, equity, statistics, and balances are fictional or research-only, with no exchange-account or real-fund access.
...
| `GET` | `/backtesting/runs` | Cursor-paginated immutable fictional simulation snapshots, newest first | Local, read-only; optional `limit=1..100` (default `50`) and UUID `cursor` from the last received item; returns `400` for an invalid or unknown cursor/limit and `503` when unavailable |
