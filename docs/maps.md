...
| milestones, next work, M1, M2, M3, M3.1, M3.2, M3.3, M3.4, M3.5, M3.6, M3.7, M3.8, M4, M4.1, M4.2, M4.3, M4.4, M4.5, M4.6, M4.7, M4.8, M4.9, original plan | `roadmap.md`, `plan.md`, `../PROJECT_CONTEXT.md` | — |
...
| risk, Risk Engine, risk assessment, maximum order notional, max notional, emergency stop, kill switch, persistent stop, operator control, authenticated control, Bearer token, token hash, SHA-256, loopback, risk control event, audit trail, Idempotency-Key, cumulative BTC position, atomic exposure, concurrency-safe exposure, daily realized loss, atomic daily loss, liquidity participation, top-of-book share, best bid quantity, best ask quantity, advisory lock, transaction serialization, UTC day, net realized PnL, position quantity, order candidate, approval, rejection, RISK_MAX_ORDER_NOTIONAL_USDT, RISK_EMERGENCY_STOP, RISK_MAX_BTC_POSITION_QUANTITY, RISK_MAX_DAILY_REALIZED_LOSS_USDT, RISK_MAX_TOP_OF_BOOK_PARTICIPATION_RATE, RISK_CONTROL_TOKEN_SHA256 | `risk-engine.md`, `roadmap.md`, `decisions.md`, `../AGENTS.md` | `../src/modules/risk-engine`, `../src/modules/paper-trading/application/paper-trading.executor.ts`, `../src/modules/paper-trading/application/paper-position-calculator.ts`, `../src/modules/paper-trading/infrastructure/prisma-paper-execution.repository.ts`, `../src/config/environment.ts`, `../compose.yaml` |
| unrealized loss limit, drawdown, rate limiting | `risk-engine.md`, `roadmap.md`, `../AGENTS.md` | Not implemented; later M4 increments |
