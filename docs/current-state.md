...
M7.34 calculates descriptive pump/correction cohort counts and exact rates as a pure internal research rule. M7.35 composes that rule over the bounded durable T+0-eligible cohort, M7.36 exposes it through a local read-only route, M7.37–M7.39 calculate, durably compose, and expose median observed magnitudes, and M7.40–M7.41 calculate and durably compose median observed pattern timing internally.
...
The following passed on 2026-09-19 after M7.41:

- `npm run build`
- `npm run lint`
- `npm run format:check`
- `npm test -- --runInBand` — 615 tests passed across 73 suites
...
M0 through M7.41 are implemented and fully verified milestone increments.
