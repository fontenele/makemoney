...
M7.34 calculates descriptive pump/correction cohort counts and exact rates as a pure internal research rule. M7.35 composes that rule over the bounded durable T+0-eligible cohort, M7.36 exposes it through a local read-only route, and M7.37 calculates exact median observed pattern magnitudes internally.
...
The following passed on 2026-09-19 after M7.37:

- `npm run build`
- `npm run lint`
- `npm run format:check`
- `npm test -- --runInBand` — 598 tests passed across 72 suites
...
M0 through M7.37 are implemented and fully verified milestone increments.
