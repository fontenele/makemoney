...
- Read-only `GET /backtesting/runs` returns immutable snapshots newest first with a validated limit from 1 through 100 and a default of 50, without recalculation or market-data access.
...
The following passed on 2026-09-13 after M6.28:

- `npm run build`
- `npm run lint`
- `npm run format:check`
- `npm test -- --runInBand` — 391 tests passed across 55 suites
- `docker compose config --quiet`
- `git diff --check`

The most recent database-backed integration validation was repeated after M6.28:

- `$env:RISK_MAX_BTC_POSITION_QUANTITY='1'; npm run test:e2e -- --runInBand` — 36 tests passed across 3 suites, including bounded deterministic recent-run listing, immutable run lookup by UUID, strict replay and simulation HTTP contracts, immutable idempotent simulation-run snapshots, conflict detection, bounded chronological stored-candle reads, exact candle storage, transactional conflict rollback, and all prior integration scenarios
...
M0 through M6.27 are committed. M6.28 changes are currently in the working tree.
