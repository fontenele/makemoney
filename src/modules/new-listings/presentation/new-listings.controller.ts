...
export class NewListingsController {
  ...
  @Get('top-of-book/imbalance/evolution')
  topOfBookImbalanceEvolutionCohort(
    @Query('limit') limit?: string,
    @Query('provider') provider?: string,
  ...
  @Get('top-of-book/spread/classification/magnitudes')
  topOfBookSpreadClassificationMagnitudeCohort(
    @Query('limit') limit?: string,
    @Query('provider') provider?: string,
    @Query('wideningBasisPoints') wideningBasisPoints?: string,
  ): Promise<ListingTopOfBookSpreadClassificationMagnitudeCohort> {
    return this.detections.getTopOfBookSpreadClassificationMagnitudeCohort(
      optionalProvider(provider) ?? 'binance',
      validLimit(limit, MAX_LISTING_OBSERVATION_COHORT_LIMIT),
      validSpreadThresholds(wideningBasisPoints),
    );
  ...
  @Get('top-of-book/spread/classification/timing')
  topOfBookSpreadClassificationTimingCohort(
    @Query('limit') limit?: string,
    @Query('provider') provider?: string,
    @Query('wideningBasisPoints') wideningBasisPoints?: string,
  ): Promise<ListingTopOfBookSpreadClassificationTimingCohort> {
    return this.detections.getTopOfBookSpreadClassificationTimingCohort(
      optionalProvider(provider) ?? 'binance',
      validLimit(limit, MAX_LISTING_OBSERVATION_COHORT_LIMIT),
      validSpreadThresholds(wideningBasisPoints),
    );
  ...
  async topOfBookImbalance(
    ...
    const identity = validObservationIdentity(provider, symbol);
    try {
      return await this.detections.listTopOfBookImbalance(
        identity.provider,
        identity.symbol,
      );
    } catch (error) {
  ...
  @Get(':provider/:symbol/top-of-book/spread/evolution')
  async topOfBookSpreadEvolution(
    @Param('provider') provider: string,
    @Param('symbol') symbol: string,
  ): Promise<ListingTopOfBookSpreadEvolution> {
    const identity = validObservationIdentity(provider, symbol);
    try {
      const result = await this.detections.getTopOfBookSpreadEvolution(
        identity.provider,
        identity.symbol,
      );
      if (!result) {
